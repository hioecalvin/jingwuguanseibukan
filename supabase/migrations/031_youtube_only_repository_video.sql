...
begin
  ...
  if exists (
    ...
    raise exception 'Existing content is not compatible with YouTube-only playback';
  end if;
end
$preflight$;

alter table public.content
  drop constraint if exists content_legacy_video_pair_check;
...
begin
  ...
  if pair_constraint_validated is distinct from true then
    raise exception 'YouTube-only content constraint postflight failed';
  end if;
end
$postflight$;

comment on constraint content_youtube_video_pair_check on public.content is
  'Repository videos are either absent or an exact YouTube provider and 11-character video identifier pair.';
