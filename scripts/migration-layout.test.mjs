...
test('only the ordered Supabase directory is active and versions 006-047 are contiguous', async () => {
  const files = (await readdir(activeDirectory))
    .filter((name) => name.endsWith('.sql'))
    .sort();
  const versions = files.map((name) => Number.parseInt(name.slice(0, 3), 10));

  assert.deepEqual(versions, Array.from({ length: 42 }, (_, index) => index + 6));
